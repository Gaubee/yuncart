...
class Listing extends Base{
	...
	public function index() {
		...
		if($price1) $wherea .= " AND a.price > '$price1'";
		if($price2) $wherea .= " AND a.price < '$price2'";
		
		
		//品牌
		$brandid	= isset($_GET['brandid'])?intval($_GET['brandid']):0;
		$this->data['brandid'] = $brandid;
		if($brandid) $wherea .= " AND a.brandid='$brandid'";
		...
		if($typeid) {
			//该类目对应的品牌
			$this->data['brands'] = DB::getDB()->join("brand","type_brand",array("on"=>"brandid"),array("a"=>"brandid,brandname"),
														 array("a"=>"isdel=0","b"=>"typeid='$typeid'"),array("a"=>"order"));
		...
		if($count) {
			$url = url('index','listing','index',
					   "catid=$catid&brandid=$brandid&path=".$this->data['path']."&orderby=view&
	...
	private function getTypeProperty($typeid) {
		$type_property = DB::getDB()->join("type_property","type_propertyvalue",array("on"=>"propertyid"),
														array("a"=>"propertyid,propertyname","b"=>"valueid,propertyvalue"),
														array("a"=>"typeid='$typeid' AND isdp=1 AND dptype=1"),
														array("a"=>"order","b"=>"order"));
		$this->data["properties"] = array();
		if(!$type_property) return ;
