...
class Base {
	...
	public function cookieToCart() {
		...
		foreach($getcookie as $k => $v) {
			...
			if(!$type  || !$id || !$num) continue;	//参数不正确
			$sessionkey = $type.'_'.$id;
			if(isset($list[$sessionkey])) continue; //购物车已经存在，
			if($type == 'item') {
	...
	public function toCart($itemid = 0,$productid = 0,$num = 1,$silent = false) {
		...
		if($productid) { //多货品
			...
			if(!$discount) $tuan = Promotion::itemInTuan($item["itemid"]);
			$price		= $item['price'];
			...
			if(!$discount) $tuan = Promotion::itemInTuan($itemid);
		} 
		!$num && ($num = 1);
		if($num > $inventory) {
	...
	protected function getCartItems() {
		...
		//货品信息
		if($productids) {
			$products = DB::getDB()->join("product","item",	array("on"=>"itemid"),
																array("a"=>"productid,price",
																	  "b"=>"itemid,itemname,itemimg,point"),
																array("a"=>"productid in ".cimplode($productids)),null,null,"productid");
		}
		//套餐信息
		if($mealid) {
	...
	public function getHint() {
		if(!isset($_SESSION['userhint'])) return ;
