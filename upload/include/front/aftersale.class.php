...
class Aftersale extends Base {
	...
	public function trade() {
		...
		if($count) {
			...
			//订单
			$this->data['trades'] = DB::getDB()->select("trade","tradeid,receiver_name,totalfee,addtime,status",$wherestr,
	...
	public function my() {
		...
		if($count) {
			...
			//查询数据
			$this->data["afterlist"]   = DB::getDB()->join("aftersale","order",$onarr,array("a"=>"*","b"=>"itemid,itemname,itemimg,productid"),
																$where,array("a"=>"afterid DESC"),$this->data['pagearr']['limit']);
		}
