...
class Promotion {
	...
	public static function getTradeManStr() {
	   if(empty($_SESSION['cart']['rule'])) return '';
	   $ret		= array();
	...
	public static function getManRule($totalfee = '',$includestr = false) {
		...
		foreach($rules as $rule) {
			...
			if( $manorder <= $totalfee) {
				...
				if($includestr) $therule['str'] = implode(',',$strarr);
				return $therule;
			}
		}
		return false;
	...
	public static function getCoupons($uid,$type = 'canuse',$limit = '') {
		...
		$coupons = DB::getDB()->join("user_coupon","coupon",
												array("on"=>"couponid"),
												array("b"=>"couponid,endtime,deno,restrict,require","a"=>"addtime,isused,tradeid"),
												$where,array("a"=>"endtime ASC"),$limit);
		return $coupons;
	}
	...
	public static function getCoupon($uid,$couponid,$type = 'canuse') {
		...
		$coupon = DB::getDB()->joinrow("user_coupon","coupon",array("on"=>"couponid"),
																 array("b" =>"couponid,endtime,deno,restrict,require","a"=>"addtime"),
																 $where);
		return $coupon;
	}
}