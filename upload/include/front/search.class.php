...
class Search extends Base{
	...
	public function index() {
		...
		if($price1) $where .= " AND price > '$price1'";
		if($price2) $where .= " AND price < '$price2'";
		...
		if($q)	$where .= " AND itemname like '%$q%'";
		...
		if($count) {
			...
			$this->data["items"]	 = DB::getDB()->select("item","itemid,itemname,itemimg,price,bn",$where,$orderstr,
