...
class Item extends Base{
	...
	public function index() {
		...
		if(!$this->data["item"]) cerror(__("item_not_exist"));
		...
		//相关商品
		$this->data['relations']	= DB::getDB()->join("item_correlation","item",array("on"=>"fitemid,itemid"),
															array("a"=>"fitemid","b"=>"itemname,itemimg,price"),
															array("a"=>$where,"b"=>"isdel=0"),
															array("a"=>"order"));
	...
	private function getItemSpec($itemid=0,$typeid = 0) {
		...
		$specs = DB::getDB()->join("spec","type_spec",array("on"=>"specid"),
	...
	private function getItemProperty($itemid) {
		...
		foreach($properties as $key=>$val) {
			if(!isset($t_keys[$key])  || (!isset($t_vals[$val['propertyvalueid']]) && !$val['self'] )  ) continue;
	...
	private function getHistoryList($theitemid) {
		...
		if(count($history) > 5) $history = array_slice($history,0,5);
		$itemids = array();
		foreach($history as $k=>$v) {
			if(!$v || ($v == $theitemid)) continue;
			$itemids[$v] = intval($v);
		}
		if($itemids) {
	...
	public function bigimg() {
		...
		if(!$this->data["itemimgs"]) cerror(__("itemimg_not_exist"));
	...
	public function getOther() {
		$type = trim($_GET["type"]);
		if(!in_array($type,array('sale','qa','comment'))) exit('failure');
		$func = "get".$type;
		$this->$func();
	}
	...
	public function getSale() {
		...
		if($count) {
			...
			$this->data['salelist']	 = DB::getDB()->select("sales","uname,price,num,saletime",$where,
															  "saleid DESC",$this->data['pagearr']['limit']);
		}
		$this->data['itemid'] = $itemid;
	...
	public function saveQa() {
		//如果用户未登录，设置为error
		if(empty($_SESSION['uid'])) exit('error_access');
	...
	public function saveNotify() {
		...
		if(empty($_SESSION['uid'])) {
			...
			if($productid) {
                ...
                //货品spec
                $productspecs = DB::getDB()->join("type_spec","product_spec",
                    array("on"=>"typeid"),
                    array("b"=>"specid,specvalid"),
                    array("b"=>"productid=$productid"),
                    array("a"=>"order"),'','specid');
