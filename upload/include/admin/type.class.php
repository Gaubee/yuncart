			...
			if($dptype ==3) { //如果是手动输入
				...
				foreach($values as $valueid => $value) {
					...
					if( cstrpos($valueid,"key_") ) { //更新
						...
						DB::getDB()->update("type_propertyvalue",
											   array("propertyvalue"=>$value,"order"=>$order),
											   "valueid='$valueid'");
				...
				//增加
				if($adddata) DB::getDB()->insertMulti("type_propertyvalue",$adddata);
				//删除
				if($dbexists) DB::getDB()->delete("type_propertyvalue","valueid in ".cimplode(array_keys($dbexists)));
			}
			

			//保存属性
			DB::getDB()->update("type_property",
									array("propertyname"=>$propertyname,"dptype"=>$dptype,"isdp"=>$isdp,"selval"=>implode(",",$selval)),
									$where);
		...
		switch($opertype){
			...
			case 'edit':
				...
				if($typeid){
					...
					//删除
					if($dbexist) DB::getDB()->delete("type_spec","typeid='$typeid' AND specid in".cimplode(array_keys($dbexist)));
					if($adddata) DB::getDB()->insertMulti("type_spec",$adddata);
