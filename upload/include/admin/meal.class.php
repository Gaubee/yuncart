...
class Meal extends Base{
		...
		switch($opertype){
			...
			case 'edit':
				...
				if($mealid) {
						...
						foreach($itemids as $itemid) {
							...
							if(cstrpos($itemid,"key")) { //修改
								...
								DB::getDB()->update("meal_item",	array("order"	=> $order,
																	...
																	"mealid='$mealid' AND itemid='$itemid'");
					...
					if($adddata)  DB::getDB()->insertMulti("meal_item",$adddata);
					if($dbexists) DB::getDB()->delete("meal_item","mealid='$mealid' AND itemid in ".cimplode(array_keys($dbexists)));
	...
	public function order() {
		...
		if($count) {
		   ...
		   //查询数据
		   $data["records"]   = DB::getDB()->join("trade_promotion","meal",$onarr,
								array("a"=>"*","b"=>"title"),
								array("a"=>$where),
