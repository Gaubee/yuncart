...
class Importtaobao {
	...
	public function importitem($itemstr,$catid) {
		...
		if(!$cid || !preg_match("/[0-9]+/",$cid)) return true;//如果不是正确的记录，直接返回true
		...
		if($imgdata) DB::getDB()->insertMulti("item_img",$imgdata);
		...
		if(strlen($iteminfo[$this->map[$this->version]['saleprop']])>4) {
			...
			foreach($tempspecs as $tempspec) {
				if(!preg_match("/(.+?):([0-9]+?):(.*?)?:(.+)/",$tempspec,$match)) continue;
				...
				foreach($thespecs as $spec) {
					...
					if(!$specid || !$specvalid || !isset($the_specs[$specid]) || !isset($the_specvals[$specvalid])  ) continue;
			...
			if($specdata) DB::getDB()->insertMulti("product_spec",$specdata);
			if($specself) {
				$selfdata = array();
	...
	//					$the_value	  = isset($the_specvals[$value])?$the_specvals[$value]:0;
			...
			if($propertydata) DB::getDB()->insertMulti("item_property",$propertydata);
		}
	...
	public function importcid($cid) {
		...
		if(!isset($req['itemprops_get_response']['item_props']['item_prop'])) return $typeid;//不存在属性需要导入
