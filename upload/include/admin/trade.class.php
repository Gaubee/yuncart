...
class Trade extends Base{
	...
	private function _tradeimport($wherestr) {
		$count = DB::getDB()->selectcount("trade",$wherestr);
		if(!$count) $this->setHint(__("no_data_import"));
				...
				if(ispostreq()) {
					...
					foreach($this->data["selprintopt"] as $k=>$v) {
						...
						if(cstrpos($field,"address")) {
							$addr = "";
							if($table == "sender") {
								$addr = getDistrict($this->data[$table]["province"],
													 $this->data[$table]["city"],
													 $this->data[$table]["district"]);
							} else if($table == "receiver"){
								$addr = getDistrict($this->data[$table]["receiver_province"],
													 $this->data[$table]["receiver_city"],
													 $this->data[$table]["receiver_district"]);
							}
