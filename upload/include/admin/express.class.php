			...
			case 'edit':
				...
				if($feetype == "self") {
					...
					foreach($hideprovs as $key=>$hideprov) {
						...
						if(!$price || !$hideprov) continue;
						$adddata[] = array("wayid"=>$wayid,"price"=>$price,"province"=>$hideprov);
					}
					//如果是修改
					if($opertype == "edit")  DB::getDB()->delete("express_prov","wayid='$wayid'");
					if($adddata) DB::getDB()->insertMulti("express_prov",$adddata);
				}
		...
		switch($opertype){
			...
			case 'edit':
				...
				if($tplid){
					...
					if(!$tplid) $this->setHint(__("add_failure",$text),"express_tpl");
