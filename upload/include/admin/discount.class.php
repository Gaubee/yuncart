...
class Discount extends Base{
	 ...
	 */
	 public function discountedit() {
		...
		$this->data["discountitems"] = DB::getDB()->join("discount_item","item",
				array("on"=>"itemid"),
				array("a"=>"discountid,itemid,limit,discount","b"=>"itemimg,price,itemname"),
				array("a"=>"discountid='$discountid'"));
		...
		switch($opertype){
			...
			case 'edit':
				...
				if($discountid){
					...
					if($discounts) {
						foreach($discounts as $key => $discount) {
							if(cstrpos($key,"key")) { //更新
								...
								DB::getDB()->update( "discount_item",
														array("discount"=>getPrice($discount,2,'int'),"limit"=>$limits[$key]),
														"discountid='$discountid' AND itemid='$itemid'");
								unset($exists[$itemid]);
							} else {	//增加
					...
					if($exists) DB::getDB()->delete("discount_item",
													  array("discountid"=>$discountid,"itemid"=>"in ".cimplode(array_keys($exists))));
					if($adddata)  DB::getDB()->insertMulti("discount_item",$adddata);
	...
	public function order() {
		...
		$where['type'] = 'discount';
		if($discountid) $where["typeid"] = $discountid;
		...
		if($count) {
		   ...
		   $this->data["pagearr"] = getPageArr($page,$pagesize,$count);
		   //查询数据
		   $this->data["records"]   = DB::getDB()->join("trade_promotion","discount",$onarr,
								array("a"=>"*","b"=>"subject"),
								array("a"=>$where),
								array("a"=>"addtime DESC"),  
