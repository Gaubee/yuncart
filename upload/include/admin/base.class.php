...
class Base {
	...
	public function output($filename) {
		...
		if(!isset($this->data['leftcur'])) $this->data['leftcur'] = $this->getLeftCur();
	...
	protected function getCatOption($data = array(),$level = 0,$selected = 0,$disabled=false){
		...
		foreach($data as $key => $val) {
			...
			if( $selected == $val["catid"] ) $html .= " selected";//是否选中
			if( $disabled && isset( $val['children'] ) ) $html .= " disabled";//不可选择

			$html .= ">" . str_repeat("&nbsp;&nbsp;",$level) . ($level ? "|_":"") . $val["catname"] . "</option>"; //option结束
			
			if( isset($val["children"]) ) $html .= $this->getCatOption($val["children"],$level + 1,$selected,$disabled);
		}
		return $html;
	}
	...
	protected function getTypeBrands($typeid,$selected = 0) {
		$brands = DB::getDB()->join("type_brand","brand",
										array("on"=>"brandid"),
										array("b"=>"brandid,brandname"),
										array("a"=>"typeid='$typeid'","b"=>"isdel=0"));
	...
	protected function adminlog($logname = '',$dodata = array()) {
		if(!getConfig("adminlog_status",0)) return ;
		$adminid = $_SESSION['admin']['adminid'];
		$uname	 = $_SESSION['admin']['uname'];
		
		$data = array();
		if(isset($dodata['do'])) $data[] = __($dodata['do']);
