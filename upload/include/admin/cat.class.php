	...
	public function catitem() {
		...
		if($scatid == 0) {
			...
			if($count) {
				...
				$this->data["items"] = DB::getDB()->join("item_cat","item",$jpara,$fields,$where,array("b"=>"order"),
				...
				if($opertype == "add") {
					if($itemids) {
						foreach($itemids as $itemid) {
							if(isset($exists[$itemid]) && in_array($catid,$exists[$itemid])) continue;
							$adddata[] = array("itemid"=>$itemid,"catid"=>$catid);
						}
						if($adddata) $ret = DB::getDB()->insertMulti("item_cat",$adddata);
					}
