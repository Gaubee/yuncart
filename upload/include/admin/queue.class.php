...
class Queue extends Base{
	...
	public function email() {
		...
		if($this->data['count']) {
			$this->data["queueemail"] = DB::getDB()->join( "notify_queue","notify_content",$jpara,
											 array("a"=>"contentid,notify,addtime,msg","b"=>"subject"),
											 $where,
											 array("a"=>"contentid DESC"),
											 "10");
		}
	...
	public function sms() {
		...
		$this->data['count']		= DB::getDB()->joincount("notify_queue","notify_content",$jpara,$where);
		if($this->data['count']) {
			$this->data["queuesms"] = DB::getDB()->join("notify_queue","notify_content",$jpara,
											array("a"=>"contentid,notify,addtime,msg","b"=>"content"),
											$where,array("a"=>"contentid DESC"),"10");
		}
