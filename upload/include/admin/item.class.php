...
class Item extends Base {
	...
	public function index() {
		...
		if($type == "zero") { 
			...
			if($count) {
				...
				$tmp = DB::getDB()->join("item","product",$join,array("a"=>"itemid"),$where,
											array("a"=>"order,itemid DESC"),$this->data['pagearr']['limit'],"itemid",true);

				$itemids			 = array_keys($tmp);
				$this->data['items'] = DB::getDB()->select("item","*",
																	 "itemid in ".cimplode($itemids),"order,itemid DESC");
			}
		}  else {
			//搜索
	...
	public function itemadd() {
		...
		if($typeid) {
			...
			$this->data["specs"]	= DB::getDB()->join("spec","type_spec",$jpara,
															array("a"=>"specid,name,memo,type"),
															array("b"=>"typeid='$typeid'"),
															array("b"=>"order"),null,"specid");
			//规格对应的规格值
			if($this->data["specs"]) {
	...
	public function itemedit() {
		...
		if($item) {
			...
			if($typeid) {
				//类目对应规格spec
				$this->data["specs"]= DB::getDB()->join("spec","type_spec",array("on"=>"specid"),
										array("a"=>"specid,name,memo,type"),
										array("b"=>"typeid='$typeid'"),
										array("b"=>"order"),null,"specid");
				if($this->data["specs"]) {
		...
		switch($opertype) {
			...
			case 'add':
				...
				if($itemid) {	
					...
					if($adddata) DB::getDB()->insertMulti("item_img",$adddata);
					if($dbimgs) DB::getDB()->delete("item_img","imgid in ".cimplode(array_keys($dbimgs)));
					...
					if($adddata) DB::getDB()->insertMulti("item_cat",$adddata);
					if($dbcats) DB::getDB()->delete("item_cat","catid in ".cimplode($dbcats));
					...
					if($adddata) DB::getDB()->insertMulti("item_property",$adddata);
					...
					if($adddata) DB::getDB()->insertMulti("item_tag",$adddata);
					if($dbtags) { //删除标签
					...
					foreach($fitemids as $key=>$fitemid) {
						$order ++;
						if(isset($dbfitemids[$fitemid])) unset($dbfitemids[$fitemid]);
					...
					if($replacedata) DB::getDB()->replaceMulti("item_correlation",$replacedata);
					...
					if($adddata) DB::getDB()->insertMulti("item_spec",$adddata);
					if($dbself) DB::getDB()->delete("item_spec","itemid=$itemid AND specid in ".cimplode(array_keys($dbself)));
					...
					if($pbns) {
						...
						if($adddata) DB::getDB()->insertMulti("product_spec",$adddata);
						...
						if($specids) {
							foreach($specids as $specid) {
								//如果存在图片，记录用户自定义的图片，
								$seri_self = isset($selfpic[$specid])?serialize(array("text"=>$self[$specid],"pic"=>$selfpic[$specid]))
																	 :serialize(array("text"=>$self[$specid]));
