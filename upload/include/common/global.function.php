...
function cimplode($str) {
	if(!is_array($str)) return "(". trim($str,",") .")";
	if($str) return "(".implode(",",$str).")";
	return "(0)";
}
...
function csetcookie($name,$value,$life = 0) {
	setcookie($GLOBALS['_CONF']['cookiepre']."_".$name,$value,$life?(time()+$life):0,$GLOBALS['_CONF']['cookiepath'],$GLOBALS['_CONF']['cookiedomain'],
...
function url($pagetype,$model,$action = '',$other = '',$html = false) {
	...
	if($pagetype == 'admin') {
		if($model == 'login' && $action!='logout') return $url;
		else return !empty($_SESSION['admin']['token'])?"{$url}{$concat}token={$_SESSION['admin']['token']}":$url;
	} 
	return $url;
}
...
function array2select($array,$valuekey = '',$textkey = '',$selected = 0) {
	...
	foreach($array as $key=>$val) {
		if(is_array($val)) {//二纬数组
			...
			if($val[$valuekey] == $selected) $html .= " selected";
			$html .= ">".$val[$textkey]."</option>";
		} else { //一纬数组
			$html .= "<option value= ".$$valuekey."";
			if($$valuekey == $selected) $html .= " selected";
			$html .= ">".$$textkey."</option>";
		}
	}
	return $html;
...
function getCommonCache($code,$type) {
	...
	if($code == 'all') return $source[$type];//全部数组
	...
	if( cstrpos($code,",") ) {//如果含有,
		...
		foreach($codearr as $key=>$val) {
			$val = strtolower($val);
			if(!isset($source[$type][$val])) continue;
			$ret[] = $source[$type][$val];
		}
		return implode(",",$ret);
